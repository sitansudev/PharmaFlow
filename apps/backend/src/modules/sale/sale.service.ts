...
export class SaleService {
  async create(
    ...
    return saleRepository.prisma.$transaction(
      async (tx) => {
        /*
        ...
        for (const item of data.items) {
          const batch =
            await tx.medicineBatch.findUnique({
              where: {
                id: item.batchId,
              },
              include: {
...
await tx.saleItem.create({
  ...
  async getById(id: string) {
    const sale =
      await saleRepository.prisma.sale.findUnique(
        {
          where: {
            id,
          },
