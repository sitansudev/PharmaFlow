...
export class MedicineService {
    async create(data) {
        ...
        const normalizedGeneric = data.genericName?.trim().toLowerCase() ?? "";
        const duplicate = existingMedicines.find((medicine) => {
            const existingGeneric = medicine.genericName
                ?.trim()
                .toLowerCase() ?? "";
        ...
        if (duplicate) {
            throw new AppError(409, `Medicine "${duplicate.name}" already exists. Select the existing medicine and add the new purchase instead.`);
        }
        return prisma.$transaction(async (tx) => {
             ...
             */
            const receivedQuantity = data.stock +
                data.bonus;
    ...
    async findGroupedById(id) {
        const medicine = await medicineRepository.findById(id);
        if (!medicine) {
            throw new AppError(404, "Medicine not found");
        }
        const normalizedName = medicine.name.trim();
        const normalizedGeneric = medicine.genericName?.trim() ??
            null;
        ...
        return {
            ...
            medicines,
            totalStock: medicines.reduce((total, item) => total +
                item.stock, 0),
            totalBatches: medicines.reduce((total, item) => total +
                item.batches
                    .length, 0),
            totalSuppliers: new Set(medicines.flatMap((item) => item.batches.map((batch) => batch.supplierId))).size,
        };
    }
    async update(id, data) {
        ...
        return prisma.$transaction(async (tx) => {
             ...
             */
            const latestBatch = existingMedicine
                .batches?.[0];
            if (latestBatch) {
            ...
            const latestRate = data.rate ??
                medicine.latestRate;
            const latestSupplierId = data.supplierId ??
                medicine.latestSupplierId;
            const latestBatchNo = data.batchNo ??
                medicine.latestBatchNo;
            const latestExpiryDate = data.expiryDate ??
    ...
    async quickCreate(data) {
        ...
        if (duplicate) {
            throw new AppError(409, `Medicine "${duplicate.name}" already exists.`);
        }
